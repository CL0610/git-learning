...
![git操作通用流程](https://github.com/CL0610/git-learning/blob/master/img/git操作通用流程.png)
...
![git几个核心区域间的关系](https://github.com/CL0610/git-learning/blob/master/img/git几个核心区域间的关系.png)
...
1. 删除工作区文件，并且也从暂存区也删除对应文件的记录：`git rm <file1> <file2>`;
...
2. 查看提交历史：git log；参数`-p`展开每次提交的内容差异，用`-2`显示最近的两次更新，如`git log -p -2`;
...
6. 验证已签署的标签：git tag -v <tag-name>，如`git tag -v v1.5`;
7. 创建一个轻量级标签的话，就直接使用git tag命令即可，连`-a`,`-s`以及`-m`选项都不需要，直接给出标签名字即可，如`git tag v1.5`;
8. 将标签推送到远程仓库中：git push origin <tag-name>，如`git push origin v1.5`；
...
[非常详细准确的git学习资料](https://git-scm.com/book/zh/v1/%E8%B5%B7%E6%AD%A5-%E5%85%B3%E4%BA%8E%E7%89%88%E6%9C%AC%E6%8E%A7%E5%88%B6)；

[git-cheat-sheet中文版](https://github.com/flyhigher139/Git-Cheat-Sheet#%E9%85%8D%E7%BD%AE)
